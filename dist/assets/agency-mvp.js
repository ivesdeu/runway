...
(function () {
  ...
  function wireSidebarUserMenu() {
    ...
    async function devAdminCall(payload) {
      ...
      if (!res.ok || !j.ok) {
        var msg = (j && j.error) ? String(j.error) : 'Request failed.';
        throw new Error(msg);
      }
      return j;
    }
      ...
      if (btnCreate && btnCreate.getAttribute('data-wired') !== '1') {
        ...
        btnCreate.addEventListener('click', function () {
          ...
          Promise.resolve()
            ...
            .then(function (j) {
              var orgId = j.organizationId || '';
              var orgSlug = j.slug || '';
              showDevResult(
                'dev-org-result',
                'Created org.<br><strong>Organization ID:</strong> <code style="font-size:11px;">' +
                  escapeHtml(orgId) +
  ...
  function fetchDevAdmin(payload) {
    ...
    return c.auth.getSession().then(function (s) {
      ...
      var base = (window.__bizdashSupabaseUrl || '').replace(/\/$/, '');
      var anon = window.__bizdashSupabaseAnonKey || '';
      return fetch(base + '/functions/v1/dev-admin', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + token,
      ...
      return res.json().catch(function () { return {}; }).then(function (j) {
        if (!res.ok || !j.ok) throw new Error((j && j.error) ? String(j.error) : 'Request failed.');
        return j;
      });
    });
  }
